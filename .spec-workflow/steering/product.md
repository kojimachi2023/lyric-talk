...
- 出力: マッチ結果（再構成されたテキスト、トークンごとの一致根拠、集計）を **CLIで表示**し、同時に **DuckDB に永続化**
...
3. **Explainable Output**: トークン単位の一致根拠（どの歌詞トークンから採ったか、モーラの内訳）と集計をユーザーに提示
4. **CLI-first UX**: `register` / `match` / `query` に加えて、`corpus list` / `run list` で履歴を確認しつつ対話的に選択できる
...
- Typer + Rich を活用し、対話的な利用と非対話（スクリプト/CI）双方で扱いやすいCLI体験を提供する
...
- **Dashboard Type**: CLI（Richによるテーブル/ツリー表示） + DuckDB（結果の永続化と照会）
